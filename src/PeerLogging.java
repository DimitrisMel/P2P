...
public class PeerLogging {
	...
	void changeOptNeigh(int p1, int p2, String time) {
		String s = time + ": " + Integer.toString(p1) + " has the optimistically unchoked neighbor "
				+ Integer.toString(p2);
		LOGGER.log(Level.OFF, s);
	}
	...
	void haveMsg(int p1, int p2, String time, int pieceIndex) {
		String s = time + ": " + Integer.toString(p2) + " received the 'have' message from " + Integer.toString(p1)
				+ " for the piece " + Integer.toString(pieceIndex);
		LOGGER.log(Level.OFF, s);
	}

	void interestedMsg(int p1, int p2, String time) {
		String s = time + ": " + Integer.toString(p1) + " received the 'interested' message from "
				+ Integer.toString(p2);
		LOGGER.log(Level.OFF, s);
	}

	void notInterestedMsg(int p1, int p2, String time) {
		String s = time + ": " + Integer.toString(p1) + " received the 'not interested' message from "
				+ Integer.toString(p2);
		LOGGER.log(Level.OFF, s);
	}

	void dwnldPiece(int p1, int p2, String time, int pieceIndex) {
		String s = time + ": " + Integer.toString(p1) + " has downloaded the piece " + Integer.toString(pieceIndex)
				+ " from " + Integer.toString(p2);
		LOGGER.log(Level.OFF, s);
	}
