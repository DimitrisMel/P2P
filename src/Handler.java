...
public class Handler implements Runnable {
	...
	public void run() {
		try {
			...
			/*
			 * //check if both are same if(inpid == connPeer) System.out.println("True");
			 * 
 ...
 * To do
 * 
 * Fix log file name according to the protocol specifics i.e.
 * ‘~/project/log_peer_1001.log’. Maybe add a time stamp on the title to
 * distinguish it from different runs, so it doesn't get replaced' Delete first
 * line of log file Handle message types (choke, unchoke, etc..) Read each
 * piece, convert it into bytearray and send it Check input methods for
 * different file types to convert to bytearray Check handshake between 2 peers
 * 
 * Check the conversion from string to bytes and back same for peerid
 * 
 * 
 * NEW: Synchronize access to hashmaps:
 * https://beginnersbook.com/2013/12/how-to-synchronize-hashmap-in-java-with-
 * example/
 * 
 */
